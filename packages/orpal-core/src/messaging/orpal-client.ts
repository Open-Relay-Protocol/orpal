...
export class OrpalClient {
  ...
  async acceptMigration(contactKey: string): Promise<boolean> {
    ...
    if (oldContact && result.newContact.identityKey) {
      ...
      this.contacts.set(updated.identityKey, {
        relayOnly: updated.relayOnly,
        transportKey: updated.transportKey,
  ...
  async sendFile(contactKey: string, source: FileSource): Promise<string> {
    ...
    const message: StoredMessage = {
      id: fileId, contactKey, direction: "out", kind: "file", ts, state: "sending",
      file: { fileId, name: source.name, size: source.size, mime: source.mime, sha256: "", chunkSize, chunks, state: "transferring", transferred: 0 },
    };
    await this.store.appendMessage(message);
    this.events.emit("message", { message });
  ...
  contactState(contactKey: string): ContactState {
    return this.conns.contactState(contactKey);
  }

  private ensureConnection(contactKey: string): Promise<ContactConn> {
    return this.conns.ensureConnection(
      contactKey,
  ...
  private observeOutbound(boardId: string, msg: Outbound): void {
    if (msg.kind === "match") {
      this.conns.recordMatch(mk(boardId, msg.match_id), msg.counterparty_key);
    }
  }
  ...
  private buildEndpoint(boardId: string, matchId: string, role: "initiator" | "responder"): WebRTCEndpoint {
    ...
    const ep = new BrowserWebRTCEndpoint(matchId, role, {
      ...
      onConnectionStateChange: (state) => this.conns.handleEndpointState(key, state),
    });
    this.conns.storeEndpoint(key, ep);
    return ep;
  }
  ...
  private async handleConnected(board: BoardEntry, info: ConnectedInfo): Promise<void> {
    ...
    const conn: ContactConn = {
      ...
      boardId: board.id,
      channel,
      endpoint: this.conns.getEndpoint(matchKey) ?? null,
      state: "connected",
      fileReceiver,
    };
  ...
  private buildFileReceiver(contactKey: string): FileReceiver {
    return new FileReceiver({
      createSink: async (offer: FileOfferFrame): Promise<FileSink> => {
        const message: StoredMessage = {
          id: offer.fileId, contactKey, direction: "in", kind: "file", ts: offer.ts, state: "delivered",
          file: { fileId: offer.fileId, name: offer.name, size: offer.size, mime: offer.mime, sha256: offer.sha256, chunkSize: offer.chunkSize, chunks: offer.chunks, state: "transferring", transferred: 0 },
        };
        await this.store.appendMessage(message);
        this.events.emit("message", { message });
  ...
  private async handleIncomingFrame(contactKey: string, text: string): Promise<void> {
    ...
    if (outer.t === "sealed") {
      ...
      if (!inner) {
        this.events.emit("error", { error: new Error("sealed message failed to decrypt; dropped"), context: "handleIncomingFrame:unseal" });
        return;
      }
  ...
  private async processFrame(contactKey: string, frame: AppFrame | SealablePayload): Promise<void> {
    switch (frame.t) {
      case "text": {
        ...
        if (!already) {
          const message: StoredMessage = { id: frame.id, contactKey, direction: "in", kind: "text", text: frame.text, ts: frame.ts, state: "delivered" };
          await this.store.appendMessage(message);
          this.events.emit("message", { message });
        }
      ...
      case "key-migration": {
        ...
        if (!result.valid) {
          this.events.emit("error", { error: new Error(`Invalid key-migration from ${contactKey}: ${result.reason}`), context: "processFrame:key-migration" });
          return;
        }
        ...
        if (autoAccept && oldTk && conn && conn.state === "connected") {
          ...
          if (challenges) {
            for (const c of challenges) {
              conn.channel
                .send(encodeAppFrame({ v: 1, t: "migration-challenge", target: c.target, sealed_nonce: c.sealedNonceB64, ack_pubkey: c.ackPubkeyB64 }))
                .catch(() => {});
            }
            return;
          }
      ...
      case "migration-challenge": {
        ...
        if (conn && conn.state === "connected") {
          conn.channel
            .send(encodeAppFrame({ v: 1, t: "migration-challenge-response", target: frame.target, sealed_echo: echo }))
            .catch(() => {});
        }
        return;
      }
      case "migration-challenge-response": {
        ...
        if (!valid) {
          this.events.emit("error", { error: new Error(`Invalid challenge response from ${contactKey} (target: ${frame.target})`), context: "processFrame:migration-challenge-response" });
        }
        if (bothVerified) {
          const accepted = await this.acceptMigration(contactKey);
          if (accepted) {
            this.events.emit("migration-progress", { progress: this.migration.progress! });
          }
        }
        return;
      }
